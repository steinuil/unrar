...
void ListArchive(CommandData *Cmd) {
        ...
        while (true) {
            ...
            if (Arc.IsArchive(true)) {
                ...
                if (!Bare) {
                    ...
                    mprintf(L"\n%s: ", St(MListDetails));
                    const wchar *Fmt = Arc.Format == RARFMT14
                                           ? L"RAR 1.4"
                                           : (Arc.Format == RARFMT15 ? L"RAR 4" : L"RAR 5");
                    mprintf(L"%s", Fmt);
                    if (Arc.Solid) {
                    ...
                    if (TitleShown) {
                        ...
                        if (Verbose) {
                            ...
                            mprintf(
                                ...
                                UnpSizeText,
                                PackSizeText,
                                ToPercentUnlim(TotalPackSize, TotalUnpSize),
                                VolNumText,
                                FileCount
                            );
                        } else {
    ...
    if (ArcCount > 1 && !Bare && !Technical) {
        ...
        if (Verbose) {
            mprintf(
                L"%21ls %9ls %3d%% %28ls %u",
                UnpSizeText,
                PackSizeText,
                ToPercentUnlim(SumPackSize, SumUnpSize),
                L"",
                SumFileCount
            );
        } else {
...
void ListFileHeader(
    ...
    if (Technical) {
        ...
        if (hd.FileHash.Type == HASH_CRC32) {
            mprintf(
                L"\n%12ls: %8.8X",
                hd.UseHashKey   ? L"CRC32 MAC"
                : hd.SplitAfter ? L"Pack-CRC32"
                                : L"CRC32",
                hd.FileHash.CRC32
            );
        }
        if (hd.FileHash.Type == HASH_BLAKE2) {
            ...
            mprintf(
                L"\n%12ls: %ls",
                hd.UseHashKey   ? L"BLAKE2 MAC"
                : hd.SplitAfter ? L"Pack-BLAKE2"
                                : L"BLAKE2",
                BlakeStr.c_str()
            );
        }
        ...
        if (!hd.Dir) {
            ...
            } else if (hd.WinSize % 1048576 == 0) {
                WinSize = L" -md=" + std::to_wstring(hd.WinSize / 1048576) + L"m";
            } else if (hd.WinSize >= 1024) {
        ...
        mprintf(
            L"\n%12ls: RAR %ls(v%d) -m%d%s",
            St(MListCompInfo),
            Format == RARFMT15 ? L"1.5" : L"5.0",
            hd.UnpVer == VER_UNKNOWN ? 0 : hd.UnpVer,
            hd.Method,
            WinSize.c_str()
        );
    ...
    if (Verbose) {
        if (hd.FileHash.Type == HASH_CRC32) {
            ...
            mprintf(
                hd.Dir ? L"          " : L"????????  "
            ); // Missing checksum is ok for folder, not for file.
        }
    }
    mprintf(L"%ls", Name);
}
