...
bool CreateReparsePoint(CommandData *Cmd, const wchar *Name, FileHeader *hd) {
    ...
    const DWORD BufSize =
        sizeof(REPARSE_DATA_BUFFER) + ((DWORD)SubstLength + 1) * 2 * sizeof(wchar);
    ...
    if (hd->Dir || hd->DirTarget) {
        ...
        HANDLE hFile =
            CreateFile(Name, GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (hd->RedirType == FSREDIR_JUNCTION) {
        ...
        rdb->ReparseDataLength = USHORT(
            ...
            sizeof(rdb->SymbolicLinkReparseBuffer.Flags) + (SubstLength + 1) * sizeof(WCHAR) +
            (PrintLength + 1) * sizeof(WCHAR)
        );
        rdb->Reserved = 0;
    ...
    if (!DeviceIoControl(
            ...
            rdb,
            FIELD_OFFSET(REPARSE_DATA_BUFFER, GenericReparseBuffer) + rdb->ReparseDataLength,
            NULL,
            0,
            &Returned,
            NULL
