    ...
    if (MatchFound || RefTarget || (SkipSolid = Arc.Solid) != false) {
        ...
        if (LinkEntry && (Arc.FileHead.RedirType != FSREDIR_FILECOPY)) {
            if (Cmd->SkipSymLinks && (Arc.FileHead.RedirType == FSREDIR_UNIXSYMLINK ||
                                      Arc.FileHead.RedirType == FSREDIR_WINSYMLINK ||
                                      Arc.FileHead.RedirType == FSREDIR_JUNCTION)) {
                ExtrFile = false;
            }
        ...
        if (ExtrFile) {
            ...
            if (Cmd->DisableNames) {
                uiEolAfterMsg(
                ); // Avoid erasing preceding messages by percentage indicator in -idn mode.
            }
...
#if defined(_WIN_ALL) && !defined(SFX_MODULE) && !defined(SILENT)
            if (!TestMode && !Arc.BrokenHeader && Arc.FileHead.UnpSize > 0xffffffff &&
                (Fat32 || !NotFat32)) {
                if (!Fat32) {
                    // Not detected yet.
            ...
            if (LinkEntry) {
                ...
                } else if (Type == FSREDIR_UNIXSYMLINK || Type == FSREDIR_WINSYMLINK ||
                ...
                if (Arc.FileHead.Method == 0) {
                    ...
                    try {
                        ...
                        if (Arc.FileHead.WinSize >= 0x40000000) {
                            uiMsg(
                                UIERROR_EXTRDICTOUTMEM,
                                Arc.FileName,
                                uint(
                                    Arc.FileHead.WinSize / 0x40000000 +
                                    (Arc.FileHead.WinSize % 0x40000000 != 0 ? 1 : 0)
                                )
                            );
                        }
                        throw;
            ...
            if (!Arc.FileHead.Solid) {
                AnySolidDataUnpackedWell =
                    false; // Reset the flag, because non-solid file is found.
            ...
            if (!SkipSolid && ShowChecksum) {
                if (ValidCRC) {
                    if (Command != 'P' && Command != 'I' && !Cmd->DisableNames) {
                        mprintf(
                            L"%s%s ",
                            Cmd->DisablePercentage ? L" " : L"\b\b\b\b\b ",
                            Arc.FileHead.FileHash.Type == HASH_NONE ? L"  ?" : St(MOk)
                        );
                    }
                } else {
                    if (Arc.FileHead.Encrypted && (!Arc.FileHead.UsePswCheck || Arc.BrokenHeader) &&
                        !AnySolidDataUnpackedWell) {
            ...
            if (!TestMode && (Command == 'X' || Command == 'E') && (!LinkEntry || LinkSuccess) &&
...
bool CmdExtract::ExtractFileCopy(
    File &New,
...
void CmdExtract::ExtrPrepareName(
    Archive &Arc,
    const std::wstring &ArcFileName,
    ...
    if (ArcPathLength > 0) {
        ...
        if (NameLength >= ArcPathLength && wcsnicompc(ArcPath, CurName, ArcPathLength) == 0 &&
            (IsPathDiv(ArcPath[ArcPathLength - 1]) || IsPathDiv(CurName[ArcPathLength]) ||
             CurName[ArcPathLength] == 0)) {
    ...
    if (AbsPaths) {
        ...
        } else if (CurName[0] == '_' && CurName[1] == '_') {
...
#ifndef RARDLL
bool CmdExtract::ExtrGetPassword(
    Archive &Arc,
    const std::wstring &ArcFileName,
    RarCheckPassword *CheckPwd
) {
...
void CmdExtract::ExtrCreateDir(Archive &Arc, const std::wstring &ArcFileName) {
    ...
    if (PrevProcessed) {
#if defined(_WIN_ALL) && !defined(SFX_MODULE)
        if (Cmd->SetCompressedAttr && (Arc.FileHead.FileAttr & FILE_ATTRIBUTE_COMPRESSED) != 0 &&
            WinNT() != WNT_NONE) {
            SetFileCompression(DestFileName, true);
        }
#endif
...
void CmdExtract::GetFirstVolIfFullSet(
