...
void GetRarDataPath(std::wstring &Path, bool Create) {
    ...
    HKEY hKey;
    if (RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\WinRAR\\Paths", 0, KEY_QUERY_VALUE, &hKey) ==
        ERROR_SUCCESS) {
        DWORD DataSize;
...
void NextVolumeName(std::wstring &ArcName, bool OldNumbering) {
    ...
    } else if (DotPos + 1 == ArcName.size() || CmpExt(ArcName, L"exe") || CmpExt(ArcName, L"sfx")) {
        ...
        if (!IsDigit(ArcName[DotPos + 2]) || !IsDigit(ArcName[DotPos + 3])) {
            ...
            while (++ArcName[NumPos] == '9' + 1) {
                if (NumPos == 0 || ArcName[NumPos - 1] == '.') {
                    ArcName[NumPos] =
                        'a'; // From .999 to .a00 if started from .001 or for too short names.
                    break;
                } else {
                    ArcName[NumPos--] = '0';
                }
    ...
    for (size_t I = 0; I < Name.size(); I++) {
        ...
        if (Extended) {
            ...
            if (IsPathDiv(Name[I + 1]) &&
                (Name[I] == ' ' || Name[I] == '.' && I > 0 && !IsPathDiv(Name[I - 1]) &&
                                       (Name[I - 1] != '.' || I > 1 && !IsPathDiv(Name[I - 2])))) {
                Name[I] = '_';
            }
        }
#else
...
#ifndef SFX_MODULE
static void GenArcName(
    std::wstring &ArcName,
    const std::wstring &GenerateMask,
    uint ArcNumber,
    bool &ArcNumPresent
) {
    size_t Pos = 0;
...
#    endif
            ...
            if (FieldPos == 1 && CField[FieldPos] == 2 && toupperw(Mask[I + 1]) == 'M' &&
                toupperw(Mask[I + 2]) == 'M') {
        ...
        if (I + 1 == Name.size() || IsPathDiv(Name[I + 1])) {
            while (I >= 0 && (Name[I] == '.' || Name[I] == ' ')) {
                ...
                if (Name[I] == '.') {
                    ...
                    if (I >= 1 && Name[I - 1] == '.' &&
                        (I == 1 || IsPathDiv(Name[I - 2]) || I == 3 && IsDriveLetter(Name))) {
                        break;
                    }
                }
                Name.erase(I, 1);
        ...
        if (I == 0 || I > 0 && IsPathDiv(Name[I - 1])) {
            ...
            for (uint J = 0; J < ASIZE(Devices); J++) {
                for (uint K = 0;; K++) {
                    ...
                    } else if (Devices[J][K] == 0) {
                        // Names like aux.txt are accessible without \\?\ prefix
                        // since Windows 11. Pure aux is still prohibited.
                        MatchFound =
                            s[K] == 0 || s[K] == '.' && !IsWindows11OrGreater() || IsPathDiv(s[K]);
                        break;
                    } else if (Devices[J][K] != toupperw(s[K])) {
                        break;
                    }
                }
            }
