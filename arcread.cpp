...
size_t Archive::SearchRR() {
    ...
    if (MainHead.Locator && MainHead.RROffset != 0) {
        ...
        if (Size != 0 && !BrokenHeader && GetHeaderType() == HEAD_SERVICE &&
            SubHead.CmpName(SUBHEAD_TYPE_RR)) {
            return Size;
        }
            ...
            switch (SubBlockHead.SubType) {
                ...
                case STREAM_HEAD:
                    *(SubBlockHeader *)&StreamHead = SubBlockHead;
                    ...
                    const size_t MaxStreamName20 =
                        260; // Maximum allowed stream name in RAR 2.x format.
...
size_t Archive::ReadHeader50() {
    ...
    if (Decrypt) {
        ...
        while (true) // Repeat the password prompt for wrong passwords.
        {
            ...
            bool EncSet = HeadersCrypt.SetCryptKeys(
                false,
                CRYPT_RAR50,
                &Cmd->Password,
                CryptHead.Salt,
...
#    else
        if (!uiGetPassword(UIPASSWORD_ARCHIVE, FileName, &Cmd->Password, CheckPwd)) {
            ...
            ); // Prevent archive deleting if delete after extraction is on.
            ErrHandler.Exit(RARX_USERBREAK);
        }
#    endif
                    ...
                    if (EncVersion > CRYPT_VERSION) {
                        ...
                        if (hd->Lg2Count > CRYPT5_KDF_LG2_COUNT_MAX) {
                            ...
                            if (hd->UsePswCheck) {
                                ...
                                if (bb->HeaderType == HEAD_SERVICE &&
                                    memcmp(hd->PswCheck, "\0\0\0\0\0\0\0\0", SIZE_PSWCHECK) == 0) {
                                    hd->UsePswCheck = 0;
                                }
                            }
                            hd->SaltSet = true;
                ...
                case FHEXTRA_HTIME:
                    if (FieldSize >= 5) {
                        ...
                        if (UnixTime && (Flags & FHEXTRA_HTIME_UNIX_NS) != 0) {
                            ...
                            uint ns;
                            if ((Flags & FHEXTRA_HTIME_MTIME) != 0 &&
                                (ns = (Raw->Get4() & 0x3fffffff)) < 1000000000) {
                                hd->mtime.Adjust(ns);
                            }
                            if ((Flags & FHEXTRA_HTIME_CTIME) != 0 &&
                                (ns = (Raw->Get4() & 0x3fffffff)) < 1000000000) {
...
bool Archive::ReadSubData(std::vector<byte> *UnpData, File *DestFile, bool TestMode) {
    ...
    if (SubHead.Encrypted) {
        if (Cmd->Password.IsSet()) {
            SubDataIO.SetEncryption(
                false,
                SubHead.CryptMethod,
                &Cmd->Password,
                SubHead.SaltSet ? SubHead.Salt : NULL,
                SubHead.InitV,
                SubHead.Lg2Count,
                SubHead.HashKey,
                SubHead.PswCheck
