...
inline void RARPPM_CONTEXT::decodeBinSymbol(ModelPPM* Model) {
    ...
    ushort& bs = Model->BinSumm[rs.Freq - 1]
                               [Model->PrevSuccess + Model->NS2BSIndx[Suffix->NumStats - 1] +
...
inline bool RARPPM_CONTEXT::decodeSymbol1(ModelPPM* Model) {
    ...
    if (count < (HiCnt = p->Freq)) {
        Model->PrevSuccess =
            (2 * (Model->Coder.SubRange.HighCount = HiCnt) > Model->Coder.SubRange.scale);
...
inline RARPPM_SEE2_CONTEXT* RARPPM_CONTEXT::makeEscFreq2(ModelPPM* Model, int Diff) {
    ...
    if (NumStats != 256) {
        psee2c = Model->SEE2Cont[Model->NS2Indx[Diff - 1]] + (Diff < Suffix->NumStats - NumStats) +
...
int ModelPPM::DecodeChar() {
    ...
    if (MinContext->NumStats != 1) {
        if ((byte*)MinContext->U.Stats <= SubAlloc.pText ||
            (byte*)MinContext->U.Stats > SubAlloc.HeapEnd) {
            return (-1);
        }
