...
void WideToUtf(const std::wstring &Src, std::string &Dest) {
    for (size_t I = 0; I < Src.size() && Src[I] != 0;) {
        ...
        if (c < 0x80) {
            ...
            if (c >= 0xd800 && c <= 0xdbff && I < Src.size() && Src[I] >= 0xdc00 &&
                Src[I] <= 0xdfff) {
                // Surrogate pair.
    ...
    if (c<0x80)
      ...
      if ((c>>5)==6)
      {
        ...
        if ((c>>4)==14)
        {
          ...
          if ((c>>3)==30)
          {
            if (Src.size()-I<3 || (Src[I]&0xc0)!=0x80 || (Src[I+1]&0xc0)!=0x80 ||
(Src[I+2]&0xc0)!=0x80)
            {
              Success=false;
              break;
...
#ifdef _WIN_ALL
    return CompareStringW(LOCALE_USER_DEFAULT, NORM_IGNORECASE | SORT_STRINGSORT, s1, -1, s2, -1) -
           2;
#else
    while (true) {
