...
bool ScanTree::GetFilteredMask() {
    ...
    AddEndSlash(Filter
    ); // Path separator is OS dependent, so we set it here instead of variable declaration.

    // SlashPos might point or not point to path separator for masks like 'dir*', '\dir*' or
    // 'd:dir*'
    std::wstring WildName = IsPathDiv(CurMask[SlashPos]) || IsDriveDiv(CurMask[SlashPos])
                                ? CurMask.substr(SlashPos + 1)
                                : CurMask.substr(SlashPos);
    Filter += WildName;
...
SCAN_CODE ScanTree::FindProc(FindData *FD) {
    ...
    if (!FastFindFile && !FindStack[Depth]->Next(FD, GetLinks)) {
        ...
        if (Slash != std::wstring::npos) {
            ...
            if (GetDirs == SCAN_GETDIRSTWICE && FindFile::FastFind(DirName, FD, GetLinks) &&
                FD->IsDir) {
    ...
    if (FD->IsDir && (!GetLinks || !FD->IsLink)) {
        ...
        if (Cmd != NULL &&
            (Cmd->ExclCheck(FD->Name, true, false, false) || Cmd->ExclDirByAttr(FD->FileAttr))) {
