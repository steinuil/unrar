...
#ifdef _WIN_ALL
    return CompareStringA(LOCALE_USER_DEFAULT, NORM_IGNORECASE | SORT_STRINGSORT, s1, -1, s2, -1) -
           2;
#else
...
void fmtitoa(int64 n, wchar *Str, size_t MaxSize) {
    ...
    wchar Info[10];
    if (!ThSep != 0 &&
        GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_STHOUSAND, Info, ASIZE(Info)) > 0) {
        ThSep = *Info;
    }
#elif defined(_UNIX)
