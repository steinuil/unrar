...
bool MergeArchive(Archive &Arc, ComprDataIO *DataIO, bool ShowFileName, wchar Command) {
    ...
    bool SplitHeader = (HeaderType == HEAD_FILE || HeaderType == HEAD_SERVICE) && hd->SplitAfter;

    if (DataIO != NULL && SplitHeader) {
        bool PackedHashPresent =
            Arc.Format == RARFMT50 || hd->UnpVer >= 20 && hd->FileHash.CRC32 != 0xffffffff;
...
bool DllVolChange(CommandData *Cmd, std::wstring &NextName) {
    ...
    if (Cmd->Callback != NULL) {
        ...
        if (Cmd->Callback(UCM_CHANGEVOLUMEW, Cmd->UserData, (LPARAM)NameBuf.data(), RAR_VOL_ASK) ==
            -1) {
            DllVolAborted = true;
        } else {
            ...
            if (OrgNextName != NextName) {
                ...
                std::copy(
                    NextNameA.data(),
                    NextNameA.data() + NextNameA.size() + 1,
                    NameBufA.begin()
                );
