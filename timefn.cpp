    ...
    if (SystemTimeToFileTime(&st, &lft)) {
        ...
        if (WinNT() < WNT_VISTA) {
            ...
            FileTimeToSystemTime(
                &lft,
                &st2
            ); // st2 might be unequal to st, because we added lt->Reminder to lft.
...
uint RarTime::GetDos() {
    RarLocalTime lt;
    GetLocal(&lt);
    uint DosTime = (lt.Second / 2) | (lt.Minute << 5) | (lt.Hour << 11) | (lt.Day << 16) |
                   (lt.Month << 21) | ((lt.Year - 1980) << 25);
    return DosTime;
}
    ...
    if (IsSet()) {
        ...
        if (FullMS) {
            swprintf(
                ...
                lt.Minute,
                lt.Second,
                lt.Reminder * (1000000000 / TICKS_PER_SECOND)
            );
        } else {
            swprintf(
                DateStr,
