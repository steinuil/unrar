...
bool File::Open(const std::wstring &Name, uint Mode) {
    ...
    if (hNewFile == FILE_BAD_HANDLE) {
        ...
        if (GetWinLongPath(Name, LongName)) {
            hNewFile =
                CreateFile(LongName.c_str(), Access, ShareMode, NULL, OPEN_EXISTING, Flags, NULL);
...
#        if defined(_APPLE)
    if (ftm != NULL) {
        ftm->SetUnixNS(st.st_mtimespec.tv_sec * (uint64)1000000000 + st.st_mtimespec.tv_nsec);
    }
    if (ftc != NULL) {
        ftc->SetUnixNS(st.st_ctimespec.tv_sec * (uint64)1000000000 + st.st_ctimespec.tv_nsec);
    }
    if (fta != NULL) {
        fta->SetUnixNS(st.st_atimespec.tv_sec * (uint64)1000000000 + st.st_atimespec.tv_nsec);
    }
#        else
    if (ftm != NULL) {
...
int64 File::Copy(File &Dest, int64 Length) {
    ...
    while (CopyAll || Length > 0) {
        Wait();
        size_t SizeToRead =
            (!CopyAll && Length < (int64)Buffer.size()) ? (size_t)Length : Buffer.size();
