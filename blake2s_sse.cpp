...
#define SSE_ROUND(m, row, r)                           \
    {                                                  \
        ...
        buf = _mm_set_epi32(                           \
            m[blake2s_sigma[r][14]],                   \
            m[blake2s_sigma[r][12]],                   \
            m[blake2s_sigma[r][10]],                   \
            m[blake2s_sigma[r][8]]                     \
        );                                             \
        G1(row[0], row[1], row[2], row[3], buf);       \
        buf = _mm_set_epi32(                           \
            m[blake2s_sigma[r][15]],                   \
            m[blake2s_sigma[r][13]],                   \
            m[blake2s_sigma[r][11]],                   \
            m[blake2s_sigma[r][9]]                     \
        );                                             \
        G2(row[0], row[1], row[2], row[3], buf);       \
...
#endif
static int
blake2s_compress_sse(blake2s_state *S, const byte block[BLAKE2S_BLOCKBYTES]) {
    __m128i row[4];
    __m128i ff0, ff1;
