    ...
    if (UnpThreadData == NULL) {
        ...
        for (uint I = 0; I < MaxItems; I++) {
            ...
            if (CurData->Decoded == NULL) {
                ...
                CurData->Decoded = (UnpackDecodedItem *)malloc(
                    CurData->DecodedAllocated * sizeof(UnpackDecodedItem)
                );
                if (CurData->Decoded == NULL) {
        ...
        while (BlockStart < DataSize && !Done) {
            ...
            for (uint Block = 0; Block < BlockNumber; Block++) {
                ...
                if (!CurData->LargeBlock && !ProcessDecoded(*CurData) ||
                    CurData->LargeBlock && !UnpackLargeBlock(*CurData) || CurData->DamagedData) {
                    Done = true;
                    break;
                }
                if (CurData->Incomplete) {
                    ...
                    CurData->BlockHeader.BlockSize -=
                        CurData->Inp.InAddr - CurData->BlockHeader.BlockStart;
...
void Unpack::UnpackDecode(UnpackThreadData &D) {
    ...
    while (true) {
        if (D.Inp.InAddr >= ReadBorder) {
            if (D.Inp.InAddr > BlockBorder ||
                D.Inp.InAddr == BlockBorder && D.Inp.InBit >= D.BlockHeader.BlockBitSize) {
                break;
            }
