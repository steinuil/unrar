...
void CommandData::ParseArg(const wchar *Arg) {
    if (IsSwitch(*Arg) && !NoMoreSwitches) {
        ...
        if (FolderArg && !Add) {
            ...
            if ((!Found || ListMode == RCLM_ACCEPT_LISTS) && ListMode != RCLM_REJECT_LISTS &&
                *Arg == '@' && !IsWildcard(Arg + 1)) {
                FileLists = true;
        ...
        case 'M':
            switch (toupperw(Switch[1])) {
                    ...
                    if (*Str == '-') {
                        ...
                        while (*Str != 0) {
                            ...
                            switch (toupperw(*(Str++))) {
                                ...
                                case 'L':
                                    Type = FILTER_LONGRANGE;
                                    break;
                                case 'X':
                                    Type = FILTER_EXHAUSTIVE;
                                    break;
                            }
                            if (*Str == '+' || *Str == '-') {
                ...
                case 'D': {
                    ...
                    if (IsDigit(LastChar)) {
                        LastChar = SetDictLimit
                                       ? 'G'
                                       : 'M'; // Treat -md128 as -md128m and -mdx32 as -mdx32g.
                    }
                    switch (LastChar) {
                        case 'K':
                            Size *= 1024;
                ...
                case 'S': {
                    ...
                    while (Pos < StoreNames.size()) {
                        ...
                        std::wstring Mask = StoreNames.substr(
                            Pos,
                            EndPos == std::wstring::npos ? EndPos : EndPos - Pos
                        );
        ...
        case 'S':
            if (IsDigit(Switch[1])) {
                ...
                switch (toupperw(Switch[1])) {
                    ...
                    case 'V':
                        Solid |=
                            Switch[2] == '-' ? SOLID_VOLUME_DEPENDENT : SOLID_VOLUME_INDEPENDENT;
                        break;
                    case 'D':
                    ...
                    case 'C': {
                        ...
                        if (!AlreadyBad) {
                            if (Switch[3] == 0) {
                                CommentCharset = FilelistCharset = ErrlogCharset = RedirectCharset =
                                    rch;
                            } else {
...
void CommandData::ProcessCommand() {
    ...
    if (Command[0] != 0 && Command[1] != 0 && wcschr(SingleCharCommands, Command[0]) != NULL ||
        ArcName.empty()) {
        OutHelp(
            Command.empty() ? RARX_SUCCESS : RARX_USERERROR
        ); // Return 'success' for 'rar' without parameters.
    }
