...
#ifdef _WIN_ALL
    return (FileAttr & (FILE_ATTRIBUTE_READONLY | FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_HIDDEN)) ==
           0;
#else
...
bool MkTemp(std::wstring &Name) {
    ...
    for (uint Attempt = 0;; Attempt++) {
        ...
        std::wstring NewName =
            Name + std::to_wstring(PID) + L"." + std::to_wstring(Ext) + L".rartemp";
...
bool LinksToDirs(
    ...
    for (size_t I = 0; I < Path.size() && I < LastChecked.size() && Path[I] == LastChecked[I];
         I++) {
            ...
            if (IsPathDiv(Path[I])) {
                ...
                if (FindFile::FastFind(Path, &FD, true) && FD.IsLink) {
...
#endif
                    {
                        ...
                        );            // Extraction command will skip this file or directory.
